...
class RandomDistortion:
    ...
    def __call__(self, results):
        ...
        img = results['img']
        if random.randint(2):
            img = torch.from_numpy(np.transpose(img, (2, 0, 1)).copy()) / 255
            img = self.tfm(img)
...
class LargeScaleJitter:
    def __init__(self,
        ...
        img_scale = (desired_size, desired_size)
        if img_scale is None:
            self.img_scale = None
        else:
            if isinstance(img_scale, list):
    ...
    def random_sample_ratio(img_scale, ratio_range):
        ...
        ratio = np.random.random_sample() * (max_ratio - min_ratio) + min_ratio
        scale = int(img_scale[0] * ratio), int(img_scale[1] * ratio)
        return scale, None, ratio

    def _random_scale(self, results):

        scale, scale_idx, ratio = self.random_sample_ratio(
            self.img_scale[0], self.ratio_range)
    ...
    def __call_random_resize(self, results):
        ...
        if 'scale' not in results:
            if 'scale_factor' in results:
                ...
                ratio = self._random_scale(results)
        else:
            if not self.override:
                assert 'scale_factor' not in results, (
                    'scale and scale_factor cannot be both set.')
    ...
    def __call_pad(self, results, out_desired_size):
        self._pad_img(results, out_desired_size)
        self._pad_masks(results)
        self._pad_seg(results)
        return results

    def _pad_img(self, results, size):
        pad_val = self.pad_val.get('img', 0)
        for key in results.get('img_fields', ['img']):
            padded_img = mmcv.impad(
                    results[key], shape=size, pad_val=pad_val)
            results[key] = padded_img
        results['pad_shape'] = padded_img.shape
    ...
    def __call_crop(self, results, crop_size):
        """Function to randomly crop images, bounding boxes, masks, semantic
        segmentation maps.
    ...
    def __call__(self, results):
        ...
        results, random_scale = self.__call_random_resize(results)  # resize
        image_size = results["ori_shape"][:2]
        out_desired_size = (
            int(self.desired_size * image_size[0] / max(image_size)),
            int(self.desired_size * image_size[1] / max(image_size))
        )

        if random_scale > 1:  # crop
            results = self.__call_crop(results, out_desired_size)
        elif random_scale < 1: # pad
            results = self.__call_pad(results, out_desired_size)
        return results
